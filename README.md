...
By doing it at the HAProxy level, I can stop most unwanted connections before they *even* reach my backend server (Velocity), which reduces load and potential attack surface, as Velocity uses much more resources to handle a connection than HAProxy does. (and also I have to install plugins on Velocity to do hostname-based filtering, which is *much* more work and less efficient than doing it at the HAProxy level)
